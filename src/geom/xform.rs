        ...
        for op in &self.ops {
            let m = match op.op_type {
                ...
                XformOpType::Matrix => {
                    ...
                    // have the SAME flat layout: columns stored sequentially, translation at indices 12-14
                    // So we can read directly as glam column-major
                    let v: Vec<f32> = op.values.iter().map(|&x| x as f32).collect();
                    glam::Mat4::from_cols(
                        glam::vec4(v[0], v[1], v[2], v[3]),     // col 0 (x_axis)
                        glam::vec4(v[4], v[5], v[6], v[7]),     // col 1 (y_axis)
                        glam::vec4(v[8], v[9], v[10], v[11]),   // col 2 (z_axis)
                        glam::vec4(v[12], v[13], v[14], v[15]), // col 3 (w_axis = translation)
                    )
                }
            };
...
impl<'a> IXform<'a> {
    ...
    pub fn new(object: &'a IObject<'a>) -> Option<Self> {
        if object.matchesSchema(XFORM_SCHEMA) {
            Some(Self { object })
        } else {
            None
        }
